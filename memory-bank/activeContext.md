...
- **Implementation in `src/routes/+page.svelte`:**
  ...
  - **Calculation Logic (Reactive Block `$: if (last7DaysLogs && !loading7DayLogs)`):**
    ...
    - Divides the summed totals by the number of days that _actually had log entries_ within the 7-day period.
...
- **Gemini Auto-fill Nutrition (`/food-items`):**
  ...
  - **Prompt Update (Noted May 2, 2025, verified May 10, 2025):** The prompt no longer requests `calories`. For the `carbs` field, it explicitly asks for carbohydrates _excluding_ fiber (net carbs), aligning with EU standards and the dynamic kcal calculation. The `fibers` field is requested separately. This ensures the `carbs` data field correctly represents net carbs.
...
- **Log Display Updates (`/`, May 2, 2025):**
  - Individual log items now display the base `serving_qty` and `serving_unit` _before_ the multiplier (e.g., `(100g) x1.5`). This requires the data to be present in the `food_items` table.
  ...
  - Reintroduced `Math.round()` for the _display_ of individual summed nutrients (PFC, FiS, etc.) in the daily summary row for visual clarity, while ensuring the calorie calculation uses the unrounded sums.
