<script lang="ts">
	...
	$: ({ positionPercent, barSegments } = calculateVisualization(
		minValue,
		maxValue,
		actualValue,
		displayMin,
	...
	function calculateVisualization(
		...
		const addSegment = (value: number | null, color: string) => {
			...
			if (width > 0.01) {
				// Use small threshold for floating point checks
		...
		if (totalWidth < 99.9 && barSegments.length > 0) {
...
<div class="w-full">
	...
	{#if actualValue !== null && actualValue !== undefined && !isNaN(actualValue)}
		<div class="relative w-full {BAR_HEIGHT} flex overflow-hidden rounded">
			<!-- Background Segments -->
			{#each barSegments as segment, index (index)}
		...
		<div class="mt-0.5 flex justify-between px-0.5 text-xs text-gray-500">
	...
	{:else}
		...
		<div
			class="relative w-full {BAR_HEIGHT} {GRAY_COLOR} rounded"
			title="Actual value not available"
		>
			<div class="mt-0.5 flex justify-between px-0.5 text-xs text-gray-500">
				<span>N/A</span>
				<span>N/A</span>
