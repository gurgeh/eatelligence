<script lang="ts">
	...
	import { calculateKcal, generateTargetLabel, getErrorMessage } from '$lib/utils';
		...
		return targetDefs.map((target) => {
			...
			try {
				...
				if (n2 === null) {
					...
					if (n2 === 'calories') {
						...
						if (n1 === 'protein' || n1 === 'fat' || n1 === 'carbs' || n1 === 'fibers') {
							const netCarbs =
								getNutrientTotal('carbs', effectiveTotals) -
								getNutrientTotal('fibers', effectiveTotals);
							if (n1 === 'protein') calcValue1 = value1 * kcalMultipliers.protein;
							else if (n1 === 'fat') calcValue1 = value1 * kcalMultipliers.fat;
							else if (n1 === 'carbs')
								calcValue1 = netCarbs * kcalMultipliers.carbs; // Use net carbs for kcal calc
				...
				if (n2 === null) {
					...
					if (n1 === 'omega6' && n2 === 'omega3') {
						// Special case for O6:O3
				...
				calculationError = getErrorMessage(err) || 'Calculation failed';
				actualValue = null;
			}
...
{#if isOpen}
	...
	>
		<!-- Modal Content -->
		<div
			class="z-50 max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl"
		...
		>
			<div class="mb-4 flex items-center justify-between">
				<h2 class="text-xl font-semibold" id="modal-title">
					Nutrition Target Details ({dateString})
				</h2>
				<button on:click={closeModal} class="text-2xl font-bold text-gray-500 hover:text-gray-700"
					>&times;</button
				>
			</div>
					...
					{#each calculatedTargets as target (target.id)}
						<div class="border-b pb-3">
							<div class="mb-1 flex items-center justify-between">
								...
								{:else if target.actualValue !== null}
									<span class="text-sm text-gray-700">
										Actual: {Math.round(target.actualValue)}
										{target.displayUnit}
									</span>
								{:else}
							...
							<div class="mb-2 text-xs text-gray-500">
								Target: {target.min_value ?? '-'} to {target.max_value ?? '-'}
								{target.displayUnit}
							</div>
							{#if target.calculationError}
								<div class="flex h-4 items-center justify-center rounded bg-gray-300">
			...
			<div class="mt-6 text-right">
				<button on:click={closeModal} class="rounded bg-gray-300 px-4 py-2 hover:bg-gray-400">
					Close
				</button>
			</div>
		</div>
